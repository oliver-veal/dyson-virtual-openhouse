...
export class PostProcessing extends GameObject {
    Init() {
        ...
        this.SetFXAAPixelRatio = () => {
            ...
            this.fxaaPass.material.uniforms[ 'resolution' ].value.x = 1 / ( this.game.container.offsetWidth * pixelRatio );
            this.fxaaPass.material.uniforms[ 'resolution' ].value.y = 1 / ( this.game.container.offsetHeight * pixelRatio );
        };
        this.SetFXAAPixelRatio();

        this.outlinePass = new OutlinePass( new THREE.Vector2( window.innerWidth, window.innerHeight ), this.game.scene, this.game.camera);
