        ...
        window.addEventListener("mousedown", (event) => {
            if (event.button === 0)
                if (this.selectedObject)
                    this.clickObject = this.selectedObject;
        });
            ...
            if (event.button === 0 && this.ENABLED && this.game.movement.controls.isLocked)
                if (this.selectedObject)
                    if (this.selectedObject === this.clickObject) {
                        this.game.events.Trigger("OnObjectClick", { object: this.selectedObject, slug: this.selectedObject.userData.name.split(":")[1] });
                        document.body.style.cursor = "";
                    }
    ...
    Update() {
        ...
        if (intersects.length > 0) {
            if (this.selectedObject !== intersects[0].object) {
                ...
                this.game.events.Trigger("OnObjectMouseOver", { object: this.selectedObject, slug: this.selectedObject.userData.name.split(":")[1] });
            }
        } else {
            if (this.selectedObject) {
                this.game.events.Trigger("OnObjectMouseLeave", { object: this.selectedObject, slug: this.selectedObject.userData.name.split(":")[1] });
                this.selectedObject = null;
            }
