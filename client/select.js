            ...
            if (event.button === 0)
                if (this.selectedObject)
                    if (this.selectedObject === this.clickObject)
                        this.game.events.Trigger("OnObjectClick", { object: this.selectedObject });
    ...
    Update() {
        ...
        if (intersects.length > 0) {
            if (this.selectedObject !== intersects[0].object) {
                ...
                this.game.events.Trigger("OnObjectMouseOver", { object: this.selectedObject });
            }
        } else {
            if (this.selectedObject) {
                this.game.events.Trigger("OnObjectMouseLeave", { object: this.selectedObject });
                this.selectedObject = null;
            }
