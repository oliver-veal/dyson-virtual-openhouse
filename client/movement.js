...
export class Movement extends GameObject {
    ...
    Update(delta) {
        ...
        if (this.ENABLED) {
            if (this.movement.panLeft)
                panWish += 1;

            if (this.movement.panRight)
                panWish += -1;
        }
        ...
        if (panWish == 0) {
            if (this.panSpeed > 0)
                this.panSpeed = Math.max(this.panSpeed - this.panAccel * delta, 0);
            if (this.panSpeed < 0)
                    this.panSpeed = Math.min(this.panSpeed + this.panAccel * delta, 0);
        }
    ...
    GetMovementDirection() {
        ...
        this.movement.cameraDirection.crossVectors(this.controls.getObject().up, this.movement.cameraDirection)
        ...
        if (movForward === 0 && movRight == 0)
            ...
            let angle = Math.atan2(this.movement.axisDirection.x*f.z - this.movement.axisDirection.z*f.x, this.movement.axisDirection.x * f.x + this.movement.axisDirection.z*f.z)
    ...
    MoveGround(accelDir, prevVel, dt) {
        ...
        return this.Accelerate(accelDir, prevVel, this.movementSettings.groundAccel, this.movementSettings.maxVelGround, dt);
    }
