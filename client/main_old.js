        ...
        if (child.isMesh) {
          ...
          if (child.userData)
            if (child.userData.name)
              if (child.userData.name.includes('poster')) pickableObjects.push(child)
          if (child.userData.name.includes('exhibit')) pickableObjects.push(child)
...
function getMovementDirection() {
  ...
  if (movForward === 0 && movRight == 0) direction.set(0, 0, 0)
  else {
    direction.copy(cameraDirection)
    let angle = Math.atan2(
      axisDirection.x * f.z - axisDirection.z * f.x,
      axisDirection.x * f.x + axisDirection.z * f.z,
    )
...
function animate() {
  ...
  if (controls.isReady) {
    ...
    if (grounded) velocity = MoveGround(direction, velocity, delta)
    // else
